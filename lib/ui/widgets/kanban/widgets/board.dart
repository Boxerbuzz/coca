...
class AppFlowyBoardConfig {
  ...
  const AppFlowyBoardConfig({
    ...
    this.boardMargin = EdgeInsets.zero,
  });
}
      ...
      child: Consumer<AppFlowyBoardController>(
        builder: (context, notifier, child) {
          ...
          BoardPhantomController phantomController =
              BoardPhantomController(delegate: controller, groupsState: boardState);
...
class _AppFlowyBoardContent extends StatefulWidget {
  ...
  const _AppFlowyBoardContent({
    ...
    this.footerBuilder,
    this.headerBuilder,
  }) : reorderFlexConfig = const ReorderFlexConfig(direction: Axis.horizontal, dragDirection: Axis.horizontal);
...
class _AppFlowyBoardContentState extends State<_AppFlowyBoardContent> {
  ...
  List<Widget> _buildColumns() {
    ...
    widget.dataController.groupData.asMap().entries.map(
      (item) {
        final columnData = item.value;
        final columnIndex = item.key;

        final dataSource = _BoardGroupDataSourceImpl(groupId: columnData.id, dataController: widget.dataController);
  ...
  Widget? _buildHeader(BuildContext context, AppFlowyGroupData groupData) {
    if (widget.headerBuilder == null) return null;
  ...
  EdgeInsets _marginFromIndex(int index) {
    if (widget.dataController.groupData.isEmpty) {
      return widget.config.groupMargin;
    }

    if (index == 0) {
      // remove the left padding of the first group
      return widget.config.groupMargin.copyWith(left: widget.config.boardMargin.left);
    }

    if (index == widget.dataController.groupData.length - 1) {
      // remove the right padding of the last group
      return widget.config.groupMargin.copyWith(right: widget.config.boardMargin.right);
    }
...
class _BoardGroupDataSourceImpl extends AppFlowyGroupDataDataSource {
  ...
  _BoardGroupDataSourceImpl({required this.groupId, required this.dataController});

  @override
  AppFlowyGroupData get groupData => dataController.getGroupController(groupId)!.groupData;
...
class AppFlowyBoardState extends DraggingStateStorage with ReorderDragTargetKeys {
  ...
  @override
  void insertDragTarget(String reorderFlexId, String key, GlobalObjectKey<State<StatefulWidget>> value) {
  ...
  @override
  GlobalObjectKey<State<StatefulWidget>>? getDragTarget(String reorderFlexId, String key) {
    Map<String, GlobalObjectKey>? group = groupDragTargetKeys[reorderFlexId];
    if (group != null) {
      return group[key];
    } else {
