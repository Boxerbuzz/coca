...
///
typedef DragTargetWillAccepted<T extends DragTargetData> = bool Function(T dragTargetData);

///
typedef DragTargetOnStarted = void Function(Widget, int, Size?);

typedef DragTargetOnMove<T extends DragTargetData> = void Function(T dragTargetData, Offset offset);
  ...
  Widget build(BuildContext context) {
    Widget dragTarget = DragTarget<T>(
      ...
      onMove: (detail) {
        ...
        if (detail.offset.dx > MediaQuery.of(context).size.width - widget.groupWidth) {
          final newPosition = scrollController.offset + expandDistance > maxScrollExtent
              ? maxScrollExtent
              : scrollController.offset + expandDistance;
          widget.scrollController.jumpTo(newPosition);
        }
        if (detail.offset.dx < 20) {
          final newPosition = scrollController.offset - expandDistance < minScrollExtent
              ? minScrollExtent
              : scrollController.offset - expandDistance;
          widget.scrollController.jumpTo(newPosition);
        }
...
class DragTargetAnimation {
  ...
  DragTargetAnimation({
    ...
    insertController = AnimationController(value: 0.0, vsync: vsync, duration: const Duration(milliseconds: 100));

    deleteController = AnimationController(value: 0.0, vsync: vsync, duration: const Duration(milliseconds: 1));
  }
...
class DragTargetMovePlaceholder extends StatefulWidget {
  final double height;
  final Color color;
  ...
  const DragTargetMovePlaceholder({
  ...
  @override
  State<DragTargetMovePlaceholder> createState() => _DragTargetMovePlaceholderState();
}

class _DragTargetMovePlaceholderState extends State<DragTargetMovePlaceholder> {
      ...
      (currentDragTargetIndex) {
        ...
        SchedulerBinding.instance.addPostFrameCallback((Duration duration) {
          if (currentDragTargetIndex == -1) {
            ...
            isHighlight.value = widget.dragTargetIndex == currentDragTargetIndex;
          }
        });
      },
    );
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      setState(() {
        if (widget.onWillAccept(widget.eventData.dragTargetData as T)) {
          ...
          widget.onDragStarted(widget.child, widget.eventData.index, widget.eventData.feedbackSize);
        } else {
