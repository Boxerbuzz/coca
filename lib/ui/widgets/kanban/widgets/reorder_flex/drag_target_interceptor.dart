  ...
  bool onWillAccept(
    ...
    if (dragTargetId == dragTargetData.reorderFlexId) {
      ...
      _delayOperation = Timer(const Duration(milliseconds: 100), () {
        ...
        if (index != -1) {
          Log.trace('[$OverlappingDragTargetInterceptor] move to $dragTargetId at $index');
...
mixin class CrossReorderFlexDragTargetDelegate {
...
class CrossReorderFlexDragTargetInterceptor extends DragTargetInterceptor {
  ...
  bool canHandler(FlexDragTargetData dragTargetData) {
    ...
    if (acceptedReorderFlexIds.contains(dragTargetData.reorderFlexId)) {
      ...
      /// Otherwise, it means the dragTarget was moved to another list.
      Log.trace(
      ...
      Log.trace("[$CrossReorderFlexDragTargetInterceptor] not accept ${dragTargetData.reorderFlexId}");
      return false;
    }
  }
  ...
  @override
  void onAccept(FlexDragTargetData dragTargetData) {
    Log.trace('[$CrossReorderFlexDragTargetInterceptor] Group:[$reorderFlexId] on onAccept');
  }

  @override
  void onLeave(FlexDragTargetData dragTargetData) {
    Log.trace('[$CrossReorderFlexDragTargetInterceptor] Group:[$reorderFlexId] on leave');
  }
  ...
  bool onWillAccept({
    ...
    Log.debug('[$CrossReorderFlexDragTargetInterceptor] isNewDragTarget: $isNewDragTarget, dragTargetIndex: '
        '$dragTargetIndex, reorderFlexId: $reorderFlexId');
