...
class AppFlowyGroupController extends ChangeNotifier with EquatableMixin {
  ...
  AppFlowyGroupItem? removeAt(int index, {bool notify = true}) {
    if (groupData._items.length <= index) {
      Log.error('Fatal error, index is out of bounds. Index: $index,  len: ${groupData._items.length}');
      return null;
    }
  ...
  bool move(int fromIndex, int toIndex) {
    ...
    if (groupData._items.length < fromIndex) {
      Log.error('Out of bounds error. index: $fromIndex should not greater than ${groupData._items.length}');
      return false;
    }
    ...
    Log.debug('[$AppFlowyGroupController] $groupData move item from $fromIndex to $toIndex');
  ...
  void replace(int index, AppFlowyGroupItem newItem) {
    if (groupData._items.isEmpty) {
      ...
      Log.debug('[$AppFlowyGroupController] $groupData replace $removedItem with $newItem at $index');
    }
