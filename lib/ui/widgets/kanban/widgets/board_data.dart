...
typedef OnMoveGroup = void Function(String fromGroupId, int fromIndex, String toGroupId, int toIndex);

typedef OnMoveGroupItem = void Function(String groupId, int fromIndex, int toIndex);

typedef OnMoveGroupItemToGroup = void Function(String fromGroupId, int fromIndex, String toGroupId, int toIndex);
...
class AppFlowyBoardController extends ChangeNotifier
  ...
  final List<AppFlowyGroupData> _groupData = [];
  ...
  UnmodifiableListView<AppFlowyGroupData> get groupData => UnmodifiableListView(_groupData);

  /// Returns list of group id
  List<String> get groupIds => _groupData.map((groupData) => groupData.id).toList();

  final LinkedHashMap<String, AppFlowyGroupController> _groupControllers = LinkedHashMap();

  AppFlowyBoardController({this.onMoveGroup, this.onMoveGroupItem, this.onMoveGroupItemToGroup});
  ...
  void insertGroup(int index, AppFlowyGroupData groupData, {bool notify = true}) {
    ...
    final controller = AppFlowyGroupController(groupData: groupData);
    _groupData.insert(index, groupData);
    _groupControllers[groupData.id] = controller;
    if (notify) notifyListeners();
  }
  ...
  void removeGroup(String groupId, {bool notify = true}) {
    final index = _groupData.indexWhere((group) => group.id == groupId);
    if (index == -1) {
  ...
  void moveGroup(int fromIndex, int toIndex, {bool notify = true}) {
    final toGroupData = _groupData[toIndex];
    final fromGroupData = _groupData.removeAt(fromIndex);

    _groupData.insert(toIndex, fromGroupData);
  ...
  void moveGroupItemToAnotherGroup(String fromGroupId, int fromGroupIndex, String toGroupId, int toGroupIndex) {
    ...
    if (toGroupController.items.length > toGroupIndex) {
      ...
      onMoveGroupItemToGroup?.call(fromGroupId, fromGroupIndex, toGroupId, toGroupIndex);
    }
  }
  ...
  @override
  UnmodifiableListView<ReOderFlexItem> get items => UnmodifiableListView(_groupData);
